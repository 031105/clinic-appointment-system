...
class DBClient {
    ...
    async query(text, params) {
        ...
        try {
            logger_1.logger.debug(`Executing query: ${text}`);
            if (params && params.length) {
                logger_1.logger.debug(`Query params: ${JSON.stringify(params)}`);
            }
            const start = Date.now();
            const result = await client.query(text, params);
            const duration = Date.now() - start;
            logger_1.logger.debug(`Query completed in ${duration}ms, returned ${result.rowCount} rows`);
            return result;
        }
        catch (error) {
    ...
    async getAppointments(options = {}) {
        const { limit = 10, offset = 0, patientId, doctorId, status, startDate, endDate } = options;
        let query = `
      SELECT 
        a.*,
        p.patient_id,
