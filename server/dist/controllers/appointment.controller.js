...
exports.getAppointmentMedicalRecord = exports.getDoctorAvailableSlots = exports.confirmAppointment = exports.rescheduleAppointment = exports.cancelAppointment = exports.createAppointment = exports.getAppointmentById = void 0;
...
const getAppointmentById = async (req, res) => {
    try {
      ...
      SELECT 
        ...
        mr.notes,
        mr.attachments,
        mr.follow_up_date as "followUpDate",
        mr.created_at as "createdAt"
      FROM medical_records mr
      WHERE mr.appointment_id = $1
    `;
        const medicalRecordResult = await db_client_1.default.query(medicalRecordQuery, [id]);
        // 组合结果
        ...
        logger_1.logger.error('Error in getAppointmentById:', error);
...
const createAppointment = async (req, res) => {
    try {
        ...
        const { doctorId, appointmentDateTime, type, reason, symptoms, duration = 30 // 默认30分钟
         } = req.body;
        // 验证必要字段
    ...
    `;
        const conflictResult = await db_client_1.default.query(conflictQuery, [
            doctorId,
            appointmentDateTime,
    ...
    `;
        const insertResult = await db_client_1.default.query(insertQuery, [
            patientId,
            doctorId,
            appointmentDateTime,
            endDateTime.toISOString(),
      ...
      WHERE d.doctor_id = $1
    `;
        const doctorResult = await db_client_1.default.query(doctorQuery, [doctorId]);
...
const cancelAppointment = async (req, res) => {
    try {
      ...
      RETURNING 
        ...
        updated_at as "updatedAt"
    `;
        const updateResult = await db_client_1.default.query(updateQuery, [reason, id]);
...
const rescheduleAppointment = async (req, res) => {
    ...
    `;
      ...
      RETURNING 
        ...
        updated_at as "updatedAt"
    `;
        const updateResult = await db_client_1.default.query(updateQuery, [
            appointmentDateTime,
            newEndDateTime.toISOString(),
            id
        ]);
        ...
        logger_1.logger.error('Error in rescheduleAppointment:', error);
...
const confirmAppointment = async (req, res) => {
    try {
      ...
      RETURNING 
        ...
        updated_at as "updatedAt"
    `;
        const updateResult = await db_client_1.default.query(updateQuery, [id]);
        return res.status(200).json(updateResult.rows[0]);
    }
    catch (error) {
        logger_1.logger.error('Error in confirmAppointment:', error);
...
const getDoctorAvailableSlots = async (req, res) => {
    ...
    `;
        const scheduleResult = await db_client_1.default.query(scheduleQuery, [doctorId]);
        // 获取医生在该日期的已有预约
    ...
    `;
        ...
        const appointmentsResult = await db_client_1.default.query(appointmentsQuery, [doctorId, queryDate]);
        // 获取选择日期是星期几
        const selectedDate = new Date(queryDate);
        const dayOfWeek = selectedDate.getDay(); // 0是周日，1是周一，依此类推
        // 找到该日期对应的工作时间
        const daySchedule = scheduleResult.rows.find(schedule => parseInt(schedule.day_of_week) === dayOfWeek);
        if (!daySchedule) {
        ...
        logger_1.logger.error('Error in getDoctorAvailableSlots:', error);
...
const getAppointmentMedicalRecord = async (req, res) => {
    try {
      ...
      `;
        ...
        const appointmentResult = await db_client_1.default.query(appointmentQuery, params);
    ...
    `;
        ...
        logger_1.logger.error('Error in getAppointmentMedicalRecord:', error);
        return res.status(500).json({ message: 'Failed to retrieve medical record' });
    }
};
exports.getAppointmentMedicalRecord = getAppointmentMedicalRecord;
