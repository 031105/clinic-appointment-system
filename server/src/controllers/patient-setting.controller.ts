...
export const getPatientProfile = async (req: AuthRequest, res: Response) => {
  try {
    ...
    logger.info(`Fetching profile for patient ID: ${patientId}`);
    ...
    logger.info(`Patient data fetched: ${JSON.stringify({
      id: patient.id,
    ...
    if (patient.address && typeof patient.address === 'string') {
      try {
        ...
        // 如果解析失败，保持原样
        logger.warn('Failed to parse address JSON:', e);
      }
    }
...
export const updateProfile = async (req: AuthRequest, res: Response) => {
  try {
    ...
    logger.info(`Updating profile for patient ID: ${patientId}`);
    logger.debug(`Profile update data: ${JSON.stringify({
    ...
    try {
      ...
      if (firstName || lastName || phone || address) {
        ...
        if (address) {
          ...
          try {
            if (typeof address === 'object' && address !== null) {
              ...
              logger.debug(`Object address converted to string: ${addressString}`);
            } else if (typeof address === 'string') {
              // 如果已经是字符串，直接使用
              addressString = address.trim();
              logger.debug(`Using string address: ${addressString}`);
            }
            ...
            logger.error('Error processing address:', e);
            setClauses.push(`address = $${paramIndex}`);
            queryParams.push('');
            paramIndex++;
          }
        ...
        // 打印最终SQL和参数用于调试
        logger.debug(`Final SQL query: ${userUpdateQuery.replace(/\s+/g, ' ')}`);
        ...
        if (userResult.rows.length === 0) {
          ...
          logger.debug(`Update successful, returned data: ${JSON.stringify(userResult.rows[0])}`);
        }
      }
      ...
      if (dateOfBirth || bloodType || height || weight) {
        ...
        const patientParams = [];
        if (dateOfBirth) patientParams.push(dateOfBirth);
        if (bloodType) patientParams.push(bloodType);
        if (height) patientParams.push(height);
        if (weight) patientParams.push(weight);
      ...
      const updatedProfile = await dbClient.query(updatedProfileQuery, [patientId]);
...
export const addAllergy = async (req: AuthRequest, res: Response) => {
  try {
    ...
    logger.info(`Adding allergy with data: ${JSON.stringify(req.body)}`);
    
    // Validate required fields
    if (!allergyName || !severity) {
      logger.warn('Missing required fields for allergy:', { allergyName, severity });
    ...
    logger.error('Error in addAllergy:', error);
    if (error instanceof Error) {
      logger.error('Error details:', error.message, error.stack);
    }
...
// 更新过敏信息
export const updateAllergy = async (req: AuthRequest, res: Response) => {
  try {
    const patientId = req.user!.id;
    const { id } = req.params;
    const { allergyName, severity, reaction, notes } = req.body;
    ...
    const verifyResult = await dbClient.query(verifyQuery, [id, patientId]);
    ...
    logger.error('Error in updateAllergy:', error);
...
export const deleteAllergy = async (req: AuthRequest, res: Response) => {
  try {
    ...
    await dbClient.query(deleteQuery, [id, patientId]);
    
    return res.status(200).json({ message: 'Allergy deleted successfully' });
  } catch (error) {
    logger.error('Error in deleteAllergy:', error);
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        logger.debug(`New emergency contact address processed: ${addressValue}`);
      } catch (e) {
        logger.error('Error processing new emergency contact address:', e);
        addressValue = '';
      }
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        logger.debug(`Emergency contact address processed: ${addressValue}`);
      } catch (e) {
        logger.error('Error processing emergency contact address:', e);
        addressValue = '';
      }
...
export const deleteEmergencyContact = async (req: AuthRequest, res: Response) => {
  try {
    ...
    logger.error('Error in deleteEmergencyContact:', error);
    if (error instanceof Error) {
      logger.error('Error details:', error.message, error.stack);
    }
